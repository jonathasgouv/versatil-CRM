		...
		if (isEmpty() == false && moment(document.getElementById('data').value, 'DD-MM-YYYY').isValid() == true) {
			if (document.getElementById('parcelas') == 1) {
				...
				let data = {
					...
					price        : document.getElementById('pricepayment').value,
					datepayment  : document.getElementById('data').value,
					description  : document.getElementById('description').value,
					notification : 1,
					payed        : false,
					id           : Date.now().toString()
				};
				pagamentos.insert(data);
				db.saveDatabase();
				...
				for (var i = 0; i < parseInt(parcelas); i++) {
					...
					let data = {
						nome         : document.getElementById('nome').value + ' | Parcela ' + (i + 1) + '/' + parcelas,
						price        : document.getElementById('pricepayment').value,
						datepayment  : currentnewdate,
						description  : document.getElementById('description').value,
						notification : 1,
						payed        : false,
